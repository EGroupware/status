import { E as EgwApp, d as et2_createWidget, b as et2_dialog } from '../../chunks/etemplate2-0eb045cf.js';
...
class statusApp extends EgwApp {
  static appname = 'status';
  _ring = null;
  static MISSED_CALL_TIMEOUT = egw.preference('ringingtimeout', 'status') ? parseInt(egw.preference('ringingtimeout', 'status')) * 1000 : 15000;
  /**
   * Constructor
   *
   * @memberOf app.status
  ...
  refresh() {
    let self = this; // give it a delay to make sure the preferences data is updated before refreshing
    ...
    setTimeout(function () {
      if (!callCancelled) {
        ...
        egw.json("EGroupware\\Status\\Videoconference\\Call::ajax_video_call", [data, data[0]['room']], function (_url) {
          ...
          if (app.rocketchat?.isRCActive(null, [{
            data: data[0].data
          }])) {
  ...
  _phoneMissedCallback(_from, _url) {
    ...
    return et2_dialog.show_dialog(function (_btn) {
      if (_btn == et2_dialog.YES_BUTTON) {
        ...
        let url = et2_createWidget('url-phone', {
          id: 'temp_url_phone',
          readonly: true
        }, self.et2);
        url.set_value(_url);
